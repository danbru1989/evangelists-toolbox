...
add_action( 'acf/render_field/key=field_5c6fe0d46ec45', 'bdset_output_events_quick_actions' );
/**
 * Outputs the Quick Actions
 *
 * @return void
 */
function bdset_output_events_quick_actions() {

	if ( ! get_field( 'primary_event_contact' ) && ! get_field( 'event_organization' ) ) {

		echo '<div class="bdset-notice">Please save a contact or location to see more details...</div>';
		...
		$address = bdset_get_long_address( get_field( 'physical_address', $organization->ID ) );
...
add_action( 'acf/render_field/key=field_5c6dbe661386b', 'bdset_output_events_recent_communication_entries' );
/**
 * Outputs the Recent Communication Entries
 *
 * @return void
 */
function bdset_output_events_recent_communication_entries() {

	$contact = get_field( 'primary_event_contact' );

	$communications = bdset_add_communications_query( $contact->ID, 'communication_group_communication_contact' );

	bdset_add_communications_loop( $communications );
