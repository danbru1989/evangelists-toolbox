...
add_action( 'acf/render_field/key=field_5c6b3712018a4', 'bdset_output_contacts_quick_actions' );
/**
 * Outputs the Quick Actions
 *
 * @return void
 */
function bdset_output_contacts_quick_actions() {

	if ( ! get_field( 'organization' ) ) {

		echo '<div class="bdset-notice">Please save an organization to see more details...</div>';
		...
		$address = bdset_get_long_address( get_field( 'physical_address' ) );
		$website = $organization->website;

		// $communication     = bdset_add_communications_query( get_the_ID(), 'communication_group_communication_contact', 1 ); // Breaks lower communications loop for some reason...
		echo '<div class="bdset-quick-action-wrap">';
...
add_action( 'acf/render_field/key=field_5c6e9dab848c7', 'bdset_output_contacts_recent_communication_entries' );
/**
 * Outputs the Recent Communication Entries
 *
 * @return void
 */
function bdset_output_contacts_recent_communication_entries() {

	$communications = bdset_add_communications_query( get_the_ID(), 'communication_group_communication_contact' );

	bdset_add_communications_loop( $communications, 20 );
