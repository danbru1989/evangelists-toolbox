 ...
 */
function bdset_get_long_address( $address, $string = false ) {
 ...
 * @return $address_formatted
 */
function bdset_get_short_address( $address, $state = true, $string = false ) {
 ...
 * @return $title
 */
function bdset_get_full_title_contact( $post_id, $terms_count = 0 ) {

	$post_id = ! $post_id ? get_the_ID() : $post_id;

	$categories = bdset_get_terms_list( $post_id, 'contact_category', ' / ', $terms_count );
 ...
 */
function bdset_add_communications_query( $compare_id, $meta_query_key, $query_count = 10 ) {
 ...
 * @return void
 */
function bdset_add_communications_loop( $communications, $word_count = 10 ) {

	echo '<div class="bdset-new-entry-wrap"><a class="button button-primary" href="/wp-admin/post-new.php?post_type=communication" target="_blank" alt="Add a new Communication Entry">Add New Entry</a></div>';

	if ( $communications->have_posts() ) {

		echo '<table class="bdset-communications-table"><thead><th class="date-column">Date</th><th class="type-column">Type</th><th class="notes-column">Notes</th><th class="edit-column">Edit</th></thead><tbody>';
		...
		while ( $communications->have_posts() ) {
			...
			$type = bdset_format_communication_type( $type );
		...
		echo '<div class="bdset-notice">There are no communication records to display.</div>';
	}
}
 ...
 */
function bdset_format_communication_type( $type ) {

	if ( $type === 'phone' ) {
		$type_formatted = '<span class="bdset-column-icons dashicons dashicons-phone"></span>Phone';

	} elseif ( $type === 'email' ) {
		$type_formatted = '<span class="bdset-column-icons dashicons dashicons-email"></span>Email';

	} elseif ( $type === 'letter' ) {
		$type_formatted = '<span class="bdset-column-icons dashicons dashicons-welcome-write-blog"></span>Letter';

	} elseif ( $type === 'thank_you' ) {
		$type_formatted = '<span class="bdset-column-icons dashicons dashicons-heart"></span>Thank You Note';

	} elseif ( $type === 'in_person' ) {
		$type_formatted = '<span class="bdset-column-icons dashicons dashicons-businessman"></span>In Person';

	} else {
		$type_formatted = '<span class="bdset-column-icons dashicons dashicons-format-chat"></span>' . $type;
	}
 ...
 * @return $date
 */
function bdset_get_event_start( $format = 0, $post_id = 0 ) {
 ...
 * @return $date
 */
function bdset_get_event_end( $format = 0, $post_id = 0 ) {
 ...
 * return @dates
 */
function bdset_get_event_dates( $format = 'F j, Y', $separator = ' – ', $post_id = 0 ) {

	if ( $format == 'M j, Y' ) {

		// Will eventually be deprecated.
		return bdset_get_event_short_month_dates( $format = 'M j, Y', $separator, $post_id );
	}

	if ( $format == 'M j' ) {

		// Will eventually be deprecated.
		return bdset_get_event_no_year_dates( $format = 'M j', $separator, $post_id );
	}

	if ( $format == 'l' ) {

		// Will eventually be deprecated.
		return bdset_get_event_day_dates( $format = 'l', $separator, $post_id );
	}

	// Single Day Event.
	if ( bdset_get_event_start() == bdset_get_event_end() ) {

		$dates = bdset_get_event_start( $format, $post_id );

		return $dates;
	}

	// Different Year.
	if ( bdset_get_event_start( 'Y' ) != bdset_get_event_end( 'Y' ) ) {

		$dates = bdset_get_event_start( 'F j, Y', $post_id ) . $separator . bdset_get_event_end( 'F j, Y', $post_id );
	...
	// Different Months.
	if ( bdset_get_event_start( 'F' ) != bdset_get_event_end( 'F' ) ) {

		$dates = bdset_get_event_start( 'F j', $post_id ) . $separator . bdset_get_event_end( 'F j, Y', $post_id );

		return $dates;
	}

	// Different Days.
	if ( bdset_get_event_start( 'j' ) != bdset_get_event_end( 'j' ) ) {

		$dates = bdset_get_event_start( 'F j', $post_id ) . $separator . bdset_get_event_end( 'j, Y', $post_id );
 ...
 * Íreturn @dates
 */
function bdset_get_event_short_month_dates( $format = 'M j, Y', $separator = ' – ', $post_id = 0 ) {
	...
	if ( bdset_get_event_start( 'Y' ) != bdset_get_event_end( 'Y' ) ) {
		...
		$dates = bdset_get_event_start( 'M j, Y', $post_id ) . $separator . bdset_get_event_end( 'M j, Y', $post_id );
	...
	// Different Months.
	if ( bdset_get_event_start( 'M' ) != bdset_get_event_end( 'M' ) ) {

		$dates = bdset_get_event_start( 'M j', $post_id ) . $separator . bdset_get_event_end( 'M j, Y', $post_id );
	...
	if ( bdset_get_event_start( 'j' ) != bdset_get_event_end( 'j' ) ) {
		...
		$dates = bdset_get_event_start( 'M j', $post_id ) . $separator . bdset_get_event_end( 'j, Y', $post_id );
 ...
 *  return @dates
 */
function bdset_get_event_no_year_dates( $format = 'M j', $separator = ' – ', $post_id = 0 ) {
	...
	if ( bdset_get_event_start( 'M' ) != bdset_get_event_end( 'M' ) ) {
		...
		$dates = bdset_get_event_start( 'M j', $post_id ) . $separator . bdset_get_event_end( 'M j', $post_id );
	...
	if ( bdset_get_event_start( 'j' ) != bdset_get_event_end( 'j' ) ) {
		...
		$dates = bdset_get_event_start( 'M j', $post_id ) . $separator . bdset_get_event_end( 'j', $post_id );
 ...
 * return @dates
 */
function bdset_get_event_day_dates( $format = 'l', $separator = ' – ', $post_id = 0 ) {
	...
	// Different Days.
	if ( bdset_get_event_start( 'l' ) != bdset_get_event_end( 'l' ) ) {

		$dates = bdset_get_event_start( 'l', $post_id ) . $separator . bdset_get_event_end( 'l', $post_id );
 ...
 * @return boolean
 */
function bdset_has_organization() {
