...
add_filter( 'manage_organization_posts_columns', 'bdset_change_organization_columns_header' );
/**
 ...
 * @return $columns
 */
function bdset_change_organization_columns_header( $columns ) {
...
add_action( 'manage_organization_posts_custom_column', 'bdset_change_organization_columns_content', 10, 2 );
/**
 ...
 * @return void
 */
function bdset_change_organization_columns_content( $column, $post_id ) {
	...
	if ( 'location' === $column ) {
		if ( ! get_field( 'physical_address' ) ) {
			...
			$address      = bdset_get_short_address( get_field( 'physical_address' ) );
			$address_full = bdset_get_long_address( get_field( 'physical_address' ) );
	...
	if ( 'category' === $column ) {
		if ( ! has_term( '', 'organization_category' ) ) {
			...
			$terms = bdset_get_terms_list( $post_id, 'organization_category' );
			echo '<strong>' . $terms . '</strong>';
		}
	}
}

// add_filter( 'manage_edit-organization_sortable_columns', 'bdset_organizations_sortable_columns' );
/**
 ...
 * @return $columns
 */
function bdset_organizations_sortable_columns( $columns ) {
