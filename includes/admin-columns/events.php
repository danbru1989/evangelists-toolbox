...
add_filter( 'manage_edit-event_columns', 'bdset_change_event_columns_header', 15 );
/**
 ...
 * @return $columns
 */
function bdset_change_event_columns_header( $columns ) {
...
add_action( 'manage_event_posts_custom_column', 'bdset_change_event_columns_content', 10, 2 );
/**
 ...
 * @return void
 */
function bdset_change_event_columns_content( $column, $post_id ) {

	// Start Date Column.
	if ( 'start_date' === $column ) {
		echo '<strong>' . bdset_get_event_start( 'M j, Y' ) . '</strong>';
	}

	// End Date Column.
	if ( 'end_date' === $column ) {
		echo '<strong>' . bdset_get_event_end( 'M j, Y' ) . '</strong>';
	}
	...
	if ( 'location' === $column ) {
		if ( ! get_field( 'event_organization' ) ) {
			...
			$address      = bdset_get_short_address( get_field( 'physical_address', $location->ID ), false );
			$address_full = bdset_get_long_address( get_field( 'physical_address', $location->ID ) );
	...
	if ( 'contact' === $column ) {
		if ( ! get_field( 'primary_event_contact' ) ) {
			...
			printf(
				...
				$contact->ID,
				bdset_get_full_title_contact( $contact )
			);
	...
	if ( 'category' === $column ) {
		if ( ! has_term( '', 'event_category' ) ) {
			...
			$terms = bdset_get_terms_list( $post_id, 'event_category' );
...
// add_filter( 'manage_edit-event_sortable_columns', 'bdset_events_sortable_columns' );
/**
 ...
 * @return $columns
 */
function bdset_events_sortable_columns( $columns ) {
...
// add_action( 'pre_get_posts', 'bdset_events_columns_sort_query', 1 );
/**
 ...
 * @return void
 */
function bdset_events_columns_sort_query( $query ) {
