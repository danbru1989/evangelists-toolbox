...
add_filter( 'manage_contact_posts_columns', 'bdset_change_contact_columns_header' );
/**
 ...
 * @return $columns
 */
function bdset_change_contact_columns_header( $columns ) {
...
add_action( 'manage_contact_posts_custom_column', 'bdset_change_contact_columns_content', 10, 2 );
/**
 ...
 * @return void
 */
function bdset_change_contact_columns_content( $column, $post_id ) {
	...
	if ( 'category' === $column ) {
		if ( ! has_term( '', 'contact_category' ) ) {
			...
			$terms = bdset_get_terms_list( $post_id, 'contact_category' );
	...
	if ( 'organization' === $column ) {
		if ( ! get_field( 'organization' ) ) {
			...
			$address      = bdset_get_short_address( get_field( 'physical_address', $organization->ID ) );
	...
	if ( 'communication' === $column ) {
		...
		$communication = bdset_add_communications_query( $post_id, 'communication_group_communication_contact', 1 );
...
// add_filter( 'manage_edit-contact_sortable_columns', 'bdset_contacts_sortable_columns' );
/**
 ...
 * @return $columns
 */
function bdset_contacts_sortable_columns( $columns ) {
	$columns['category']      = 'category';
	$columns['communication'] = 'communication';
	return $columns;
}

// add_action( 'pre_get_posts', 'bdset_contacts_columns_sort_query', 1 );
/**
 ...
 * @return void
 */
function bdset_contacts_columns_sort_query( $query ) {
