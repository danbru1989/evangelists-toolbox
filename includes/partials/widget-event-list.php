...
add_action(
	'widgets_init',
	function() {
		register_widget( 'bdset_Events_List_Widget' );
	}
);
/**
 * Creates the Events List widget.
 */
class bdset_Events_List_Widget extends \WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $events->have_posts() ) {
			...
			echo '<ul class="bdset-events-list" >';

			while ( $events->have_posts() ) {
				$events->the_post();

				$location         = get_field( 'event_organization' );
				$location_address = bdset_get_short_address( get_field( 'physical_address', $location->ID ), false );
				...
				echo '<a class="event-title" href="' . get_permalink() . '">' . get_the_title() . '</a><span class="datetime">' . bdset_get_event_dates() . '</span>' . $location_address;
	...
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Upcoming Meetings', bdset_PLUGIN_TEXT_DOMAIN );
		$number_of_events = ! empty( $instance['number_of_events'] ) ? $instance['number_of_events'] : __( '5', bdset_PLUGIN_TEXT_DOMAIN );
		?>
			<p>
