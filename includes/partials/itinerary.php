 ...
 */
function bdset_add_itinerary_loop() {
	...
	if ( $events->have_posts() ) {
		...
		while ( $events->have_posts() ) {
			 ...
			 */
			if ( $year !== bdset_get_event_start( 'Y' ) ) {
				...
				?>
					<h2 class="year-title"><?php echo bdset_get_event_start( 'Y' ); ?></h2>
					<div class="table">
			...
			if ( get_field( 'primary_event_contact' ) ) {
				$contact = bdset_get_full_title_contact( get_field( 'primary_event_contact' ), 1 );
			} else {
				$contact = '–';
			}
			...
			if ( get_field( 'event_display_settings' ) === 'pending' ) {
				...
				<span class="row has-special-status">
					<div class="cell column-1 date"><?php echo bdset_get_event_dates( 'M j' ); ?></div>
