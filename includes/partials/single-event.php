 ...
 * @return void
 */
function bdset_add_single_event_content() {

	$location = get_field( 'event_organization' );
	$contact  = get_field( 'primary_event_contact' );

	$location_address         = bdset_get_long_address( get_field( 'physical_address', $location->ID ) );
	$location_address_encoded = rawurlencode( bdset_get_long_address( get_field( 'physical_address', $location->ID ), true ) );
	$location_website         = $location->website;
	$contact_name             = bdset_get_full_title_contact( $contact->ID, 1 );
	...
	$dates = bdset_get_event_dates( 'F j, Y' );
	$days  = bdset_get_event_dates( 'l' );
